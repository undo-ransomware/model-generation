...
def merge_tracking_status(fs, ops, warn, delta_stats, duration_stats):
	...
	# prefer filesystem status because fileops tracking can be evaded, but
	# propagate special fileops indications
	if ops.group == 'inconsistent':
		# something weird happened. all bets are off :(
		group = 'inconsistent'
	elif fs.group == 'new' and ops.group == 'phantom':
...
def merge_status(status_manifest, warn_manifest, status_fileops, warn_fileops,
		virtual_start_time, real_start_time, virtual_stop_time, real_stop_time):
	...
	status = { path: merge_tracking_status(status_manifest.get(path),
					status_fileops.get(path), warnings[path],
					delta_stats, duration_stats)
			for path in paths}
	...
	return status, warnings, delta_stats, duration_stats, last_operation
